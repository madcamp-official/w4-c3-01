...
## 공통과제 IV : 협업형 실전 산출물 제작 (2인 1팀)

**목적:** 실시간 인터랙션, LLM Wrapper, Cross-Platform 중 하나의 옵션을 선택해 구현하고, 선택한 기술이 실제로 동작하는 산출물을 완성한다.
...
| LLM Wrapper | LLM API를 활용해 AI 기능이 포함된 산출물을 구현 |
...
ALine은 **Supabase Realtime**을 중심으로 채팅 메시지, 읽음 상태, 알림, AR 라운지의 공간 낙서와 접속 상태를 동기화한다. 사용자는 카메라 앞에서 손가락으로 그림을 그리고, QR로 같은 공간에 입장한 사용자들과 AR 낙서를 실시간으로 공유할 수 있다.
