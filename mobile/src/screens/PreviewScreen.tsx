...
export default function PreviewScreen({ navigation, route }: Props) {
  const { image, video, strokes, drawing, intent, editPostId, caption: initialCaption } = route.params;
  ...
  async function handleShare() {
    ...
    setSharing(true);
    try {
      await sharePost({ image, video, strokes, drawing, caption: caption.trim() });
        ...
        >
          {video ? (
            <VideoView
              player={videoPlayer}
              style={{ flex: 1 }}
              contentFit={intent.kind === 'post' ? 'cover' : 'contain'}
              nativeControls
