// Adapted from frontend/src/mock/store.ts — keep in sync.
// The web version rasterizes seed strokes to PNG data URLs via <canvas>
// (renderStrokesToDataURL/drawStrokesStatic in lib/canvas.ts). RN has no
// canvas, so seed images fall back to a static placeholder — this only
// matters when Supabase env vars aren't set, which shouldn't happen for the
// shipped app.
import { DEFAULT_HEART_URL } from '@/lib/defaultHeart';
import { heartStrokeNormalized, randomScribble } from '@/lib/strokes';
 ...
 * State resets on app reload — this is a prototype data layer, not storage.
 */
class MockStore {
  posts: Post[] = [];
  chats: Chat[] = [];
